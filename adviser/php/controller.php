...
if(isset($_POST['update_login_details'])){ 
  ...
  if(udi("$u adviser set rank='$rank', name='$name', dept='$dept', interest='$interest' where adviser_id='$adviser_id'")){
...
//update departmental activities
if(isset($_POST['activities'])){ 
	$act_time = filters('act_time');
	$act_title = filters('act_title');
		$act_date = filters('act_date'); 
		$act_message = filters('act_message'); 
		$adviser_id = filters('adviser_id'); 

  if(udi("$i actiivities values('','$act_title','$act_message','$act_time','$act_date','2015/2016','$adviser_id','1')")){
   	$success="Update was Sucessful!!!";
   }
   else{$error1 = "Sorry, Something went wrong!!!";}
}
...
if(isset($_POST['send_gsms'])){ 
	$msg_title = filters('msg_title');
	$gsms = filters('gsms');
		$adviser_id = filters('adviser_id');     
  if(udi("$i general_feed values('','$msg_title','$gsms',CURDATE(),'2015/2016','1','$adviser_id')")){
   	$success="Update was Sucessful!!!";
   }
   else{$error1 = "Sorry, Something went wrong!!!";}
}
...
if(isset($_POST['update_ad_data'])){ 
  ...
  if(udi("$u adviser set email='$email', username='$username', phone='$phone' where adviser_id='$adviser_id' ")){
   	$success="Update was Sucessful!!!";
   }
   else{$error1 = "Sorry, Something went wrong!!!";}
}
